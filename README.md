...
with any finite constant set, generates the elementary function
class.

### Setup

Let `ElemExpr` denote the class of two-variable expressions
generated by variables, complex constants, `+`, `−`, `·`, `exp`,
and `ln`. Every `E ∈ ElemExpr` induces a partial operator
`⟦E⟧ : ℂ × ℂ ⇀ ℂ`; partiality comes from `ln(0)` being undefined,
propagating upward.

The *tree-closure* of `(⟦E⟧, c)` over a constant interpretation
`c : ι → ℂ` is the set of functions `ℂ ⇀ ℂ` representable as
finite expression trees with leaves drawn from
`{variable} ∪ range(c)` and internal nodes applying `⟦E⟧`.

### Theorem 1 — total operators produce total closures

If `⟦E⟧` is total on `ℂ × ℂ`, every tree is total. The closure
therefore contains only total functions, and in particular cannot
contain any partial member of the elementary class (e.g. `ln`,
`1/z`, `tan`, `sec`).

*Proof.* Structural induction on trees. ∎

### Theorem 2 — the natural domain of a tree is undecidable

Let `P ⊂ E` be a property of elementary expressions for which no
algorithm decides membership — for instance, zero-equivalence, by
Richardson (1968). Suppose there is a computable map
`φ : E → Tree(ι)` such that for every `e ∈ E`, the natural domain
of `φ(e)` is all of `ℂ` if and only if `e ∉ P`. Then the
predicate "`dom(T) = ℂ`" on trees is not decidable.

*Proof.* A decider `δ` for `dom(T) = ℂ` yields `d(e) := ¬δ(φ(e))`,
a decider for `P`. Contradiction. ∎

**Consequence.** Suppose someone exhibits a tree `T` over a
partial elementary operator and claims `T` represents a total
function `f`. By Theorem 2 combined with the Elementary barrier
below: either `T` does not represent `f` pointwise, or `T`
represents `f` only on a strict subset of `ℂ` whose complement —
the inputs where `T` breaks — is undecidable. Giving up pointwise
correctness yields no algorithmically checkable approximation.

### The Elementary barrier

If `E ∈ ElemExpr` is partial, then no tree over `(⟦E⟧, c)`
pointwise represents `z ↦ −z` on `ℂ`.

*Proof sketch.* Four classical ingredients assemble.

1. **Hadamard factorization.** A nowhere-zero entire function has
   the form `exp ∘ g` for some entire `g` (Hadamard 1893 /
   Weierstrass).
2. **Monodromy constancy.** `ln(exp(g(z))) = g(z) + 2πi·k(z)` for
   some integer-valued `k`. Continuous away from branch cuts and
   integer-valued, hence constant on each connected component of
   its domain.
3. **Cancellation closure.** A pointwise-total tree over a partial
   `ElemExpr` reduces, by bottom-up application of (1) and (2), to
   a `ln`-free sub-expression — an exp-polynomial over the
   constants.
...
For every `E ∈ ElemExpr` and every constant interpretation `c`,
the tree-closure of `(⟦E⟧, c)` does not simultaneously contain a
partial function `f : ℂ ⇀ ℂ` and the function `z ↦ −z`.

*Proof.* If `⟦E⟧` is total, Theorem 1 rules out `f`. If `⟦E⟧` is
partial, the Elementary barrier rules out `z ↦ −z`. ∎

### Corollary — no Sheffer basis for the elementary functions

No pair `(⊙, S)` — with `⊙` a binary elementary operator and `S`
a finite constant set — generates the elementary function class
`𝓔` under tree composition.

*Proof.* For the closure to be a subset of `𝓔`, `⊙` must itself
be elementary, so `⊙ = ⟦E⟧` for some `E ∈ ElemExpr`. The class
`𝓔` contains both a partial function (e.g. `ln`) and the total
function `z ↦ −z`. By Theorem 3, both cannot be in the closure
of `(⟦E⟧, c)`. So the closure is a proper subset of `𝓔`. ∎
...
4. `z ↦ −z` is elementary. The claim requires it to be in the
   closure. Contradiction. ∎

Only the barrier fires; Theorem 1's total-operator branch does
not, because `eml` is manifestly partial. The combined theorem
and the general corollary would matter only if one proposed a
*total* elementary operator as a Sheffer basis.

### No model escapes

The paper's rescue attempts — restrict inputs to `ℝ₊`, extend with
`±∞`, and so on — do not evade the theorem. A single observation
collapses them.

1. The elementary function class contains `i`, `sin`, `cos`,
   `exp(iπ) + 1 = 0`, and their compositions. These are
   complex-valued.
2. `ℝ` (and `ℝ₊`) do not contain `i`. So `ℝ` is trivially not a
   model of the elementary functions — the class does not even
   fit.
3. Any model therefore contains `ℂ`.
4. By Theorem 3 on `ℂ`, no `ElemExpr` operator generates the
   class on `ℂ`.
5. So no model supports the Sheffer claim.

Extending `ℂ` with `±∞` does not help. On the `ℂ` sub-domain,
tree evaluation must agree with `⟦E⟧` wherever the latter is
defined, and Theorem 3 rules this out. Any tree escaping by
routing through `±∞` at some `ℂ`-input introduces the `∞ − ∞`
indeterminacy — a separate structural failure whose consequences
(non-confluence of the equational theory) suffice independently.

The only apparent room to manoeuvre is equivocating between "real
elementary functions" (a smaller class) and "the elementary
functions" (the full `ℂ` class, with `sin`, `cos`, `i` all
present via Euler). That is a vocabulary trick, not two
legitimate class choices. Once the class is pinned down —
standard definition, complex-valued — no model escapes.
